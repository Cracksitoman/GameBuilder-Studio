...
export const App: React.FC = () => {
  ...
  const handleToggleOrientation = () => setCanvasConfig(prev => prev.mode === 'LANDSCAPE' ? { width: 450, height: 800, mode: 'PORTRAIT' } : { width: 800, height: 450, mode: 'LANDSCAPE' });
  ...
  return (
    <div className="h-[100dvh] w-screen bg-gray-950 text-white font-sans overflow-hidden flex flex-col relative animate-in fade-in duration-500 supports-[height:100dvh]:h-[100dvh]">
          ...
          <div className="flex-1 relative flex flex-col overflow-hidden bg-gray-950">
              <div className={`flex-1 relative w-full h-full overflow-hidden ${editorMode === 'SCENE' ? 'block' : 'hidden'}`}>
                    ...
                    <div className="absolute inset-0 pb-16 bg-gray-950">
                        <Canvas objects={objects} layers={layers} selectedObjectId={selectedObjectId} currentTool={currentTool} activeBrushId={activeBrushId} brushSolid={brushSolid} activeLayerId={activeLayerId} assets={assets} canvasConfig={canvasConfig} cameraConfig={cameraConfig} onSelectObject={setSelectedObjectId} onUpdateObject={handleUpdateObjectInScene} onEditObject={setEditingObject} />
                    </div>
      ...
      <AssetManagerModal isOpen={isAssetManagerOpen} assets={assets} onClose={() => setIsAssetManagerOpen(false)} onAddAsset={(a) => setAssets([...assets, a])} onDeleteAsset={(id) => setAssets(assets.filter(a => a.id !== id))} onSelectAsset={(url) => { setIsAssetManagerOpen(false); }} />
      <VariableManagerModal isOpen={isVarManagerOpen} variables={globalVariables} onClose={() => setIsVarManagerOpen(false)} onUpdateVariables={setGlobalVariables} />
      ...
      <div className={`absolute bottom-0 left-0 right-0 h-[60%] z-[60] transform transition-transform duration-300 ease-out ${activePanel === 'library' && editorMode === 'SCENE' ? 'translate-y-0' : 'translate-y-full'}`}><ObjectLibrary objects={objects} selectedObjectId={selectedObjectId} onAddObject={handleAddObject} onSelectObject={setSelectedObjectId} onDeleteObject={(id) => updateCurrentScene({ objects: objects.filter(o => o.id !== id) })} onClose={() => setActivePanel('none')} className="rounded-t-2xl" /></div>
      <div className={`absolute bottom-0 left-0 right-0 h-[60%] z-[60] transform transition-transform duration-300 ease-out ${activePanel === 'properties' && editorMode === 'SCENE' ? 'translate-y-0' : 'translate-y-full'}`}><PropertiesPanel selectedObject={selectedObject} objects={objects} globalVariables={globalVariables} assets={assets} onUpdateObject={handleUpdateObjectInScene} onOpenAssetManager={() => setIsAssetManagerOpen(true)} onSetBrush={handleSetBrush} activeBrushId={activeBrushId} brushSolid={brushSolid} onSetBrushSolid={setBrushSolid} currentTool={currentTool} onClose={() => setActivePanel('none')} className="rounded-t-2xl" /></div>
